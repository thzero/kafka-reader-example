...
| `duplicateMessage_uniqueConstraintViolation_routesToDeadLetter_acks` | Configures `controlService.recordReceived` to throw `DataIntegrityViolationException`, then asserts the message is routed to dead letter with `DUPLICATE`, acknowledged, and that `process`/`publish` are never called. | Duplicate detection — a `DataIntegrityViolationException` from the unique constraint means the `messageId` was already processed; route to dead letter and ack to advance the offset. |
...
| No duplicate processing — unique constraint as duplicate guard | `recordReceived_duplicateMessageId_throwsDataIntegrityViolation`, `duplicateMessage_uniqueConstraintViolation_routesToDeadLetter_acks` |
...
| `ReceivedRecord` written before scheduling deferred work | `happyPath_writesControlRecords_publishesAndAcks` (verifies `recordReceived` is called) |
