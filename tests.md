...
| `bdeEvent_siphonsToSiphonTopic_acks` | Sends a message with `eventType: BDE` and asserts `kafkaProducerService.siphon()` is called, `acknowledge` is called, and `controlService`/`process`/`publish`/`deadLetterService` are never invoked. | BDE siphon — BDE events bypass the entire processing pipeline and are forwarded directly to the siphon topic on the consumer thread. |
| `bdeEvent_siphonFailure_noAck` | Configures `kafkaProducerService.siphon()` to throw `KafkaPublishException` and asserts `acknowledge` is never called and no other services are invoked. | BDE siphon failure — a failed siphon publish returns without ack so the message is redelivered. |
